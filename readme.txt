...
1. Passing the N values required implementation of special function which read this value from hdfs and setting value in configuration
2. Implementing the ascending order required overloading the comparator operator.
3. Removal of red links required implementation of two map reduce jobs which increased the complexity of debugging.


Optimized done:

1. Use of hashset to eliminate duplicate in the mapper to reduce the network activity
2. Used special string manipulations to find out the out-links
3. Used two map reduce jobs to remove red links in the first stage to reduce the file size and network activity.


What we learned:

1. Got in depth understanding of hadoop framework.
2. Running jobs on AWS using WEB interface.
3. Running jobs on AWS using CLI interface.
4. Debugging hadoop jobs on local machine.
5. Configure Hadoop as single instance.
6. Learned the Pagerank algorithm and its implementation using hadoop architecture. 


Responsibilities:

We decided not to jump into the coding right away. We designed each MapReduce task for 5 jobs. Abhijeet worked on Reducer tasks and Dhananjay worked on Mapper tasks. This gave us better understanding of how each Map and Reduce tasks work in every job. While we both worked on debugging and execution.


AWS credits used:
Total 400 instance hours
